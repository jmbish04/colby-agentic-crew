...
CF_TS_HINTS = {
    ...
    "queues": [r"\bQueue\s*\<", r"\[\[queues\]\]"],
    "workflows": [r"\bworkflows?\.register\b", r"\bWorkflows?Client\b"],
    "actors": [r"@cloudflare/actors", r"\bActors?Namespace\b"],
    "agents_sdk": [r"@cloudflare/agents", r"\bAgents?SDK\b", r"\bagent\(.+?\)"],
    "containers": [r"@cloudflare/workers-types/experimental", r"\bsandbox\b", r"\bcontainers?\b"],
    "websockets": [r'headers\.get\("Upgrade"\)\s*===\s*"websocket"'],
    "assets": [r"\bassets\s*=\s*\{", r'\bassets\s*=\s*\{[^}]*directory[^}]*\}'],
    "wrangler_toml": [r"\bcompatibility_date\b", r"\bmain\s*=\s*"],
    "ui_shadcn": [r"\bshadcn\b", r"from\s+['\"]@?\/?components\/ui\/"],
    "react_frontend": [r"\.tsx?\b", r"react", r"vite", r"next", r"tailwind"]
}

FILES_OF_INTEREST = (".ts", ".tsx", ".js", ".mjs", ".cjs", ".toml", ".json", ".md", ".yaml", ".yml")
MAX_READ = 200_000  # cap per file for safety
...
def extract_wrangler_bindings(toml_text: str) -> Dict[str, Any]:
    ...
    # naive toml scans (no toml lib dependency)
...
def collect_repo(repo_dir: Path) -> Dict[str, Any]:
    meta = {
        ...
        "wrangler": {},
        "packages": [],
        "features": {k: False for k in CF_TS_HINTS.keys()},
        "files_scanned": 0,
        "notable_paths": [],
    }
    ...
    for p in repo_dir.rglob("*"):
        ...
        # keep tiny samples to feed LLM context (cap)
        if p.suffix in (".ts", ".tsx", ".toml") and len(aggregate_text_sample) < 30:
            # extract a few interesting lines
            for pat in (r"class\s+\w+\s+extends\s+DurableObject",
                        r"\[\[.*?\]\]", r"binding\s*=", r"class_name\s*=",
                        r"from\s+['\"][^'\"]+['\"]"):
...
def write_docs(repo_name: str, meta: Dict[str, Any]) -> None:
    ...
    md = DOCS_ROOT / f"{repo_name}_overview.md"
    js = DOCS_ROOT / f"{repo_name}_artifacts.json"
    md.write_text(render_md(repo_name, meta), encoding="utf-8")
...
def send_to_proxy(repo_name: str, meta: Dict[str, Any]) -> Optional[str]:
    ...
    try:
        ...
        return f"Proxy summarize failed: {e}"

def main():
    repos = [p for p in VENDOR_ROOT.iterdir() if p.is_dir()]
    index = []
    for repo_dir in repos:
        meta = collect_repo(repo_dir)
        write_docs(repo_dir.name, meta)
        # optional: store LLM md alongside the overview
